...
public final class MagicUtil {
	...
	private static void fillBlockingPieces() {
		...
		for (int i = 0; i < 64; i++) {
			// up
			if (i < 56) {
				neighbourPieces[i] |= Util.POWER_LOOKUP[i + 8];
				// up-right
				if ((i & 7) != 0) {
					neighbourPieces[i] |= Util.POWER_LOOKUP[i + 7];
				}
				// up-left
				if ((i & 7) != 7) {
					neighbourPieces[i] |= Util.POWER_LOOKUP[i + 9];
				}
			}
			// right
			if ((i & 7) != 0) {
				neighbourPieces[i] |= Util.POWER_LOOKUP[i - 1];
			}
			// left
			if ((i & 7) != 7) {
				neighbourPieces[i] |= Util.POWER_LOOKUP[i + 1];
